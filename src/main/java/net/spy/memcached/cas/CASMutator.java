...
import net.spy.memcached.MemcachedClientWithTranscoder;
...
public class CASMutator<T> extends SpyObject {
	...
	private final MemcachedClientWithTranscoder client;
	 ...
	 */
	public CASMutator(MemcachedClientWithTranscoder c, Transcoder<T> tc, int max_tries) {
		super();
		client=c;
		transcoder=tc;
		max=max_tries;
	 ...
	 * @param max_tries the maximum number of attempts to get a CAS to succeed
	 */
	public CASMutator(MemcachedClientWithTranscoder c, Transcoder<T> tc) {
		this(c, tc, MAX_TRIES);
	}
